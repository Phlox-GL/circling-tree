...
{}
  ...
  :files $ {}
    'app.comp.bezier-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-bezier-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-bezier-demo (states)
            let
    ...
    'app.comp.chars-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-char $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-char (touch-key kind)
            container ({})
              rect $ {}
                :position $ [] -3 -3
                :size $ [] 46 46
    ...
    'app.comp.chord-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-chord-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-chord-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
              ...
              container ({})
                graphics $ {}
                  :position $ [] 200 320
                  :ops $ generate-ops $ option:unwrap-or (get state :size) nil
    ...
    'app.comp.circle-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-circle-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-circle-demo (touch-key)
            container
              {} $ :position $ [] 0 140
              comp-reset $ [] -80 -100
              create-list :container ({})
    ...
    'app.comp.cycloid-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-cycloid-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-cycloid-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
                ...
                graphics $ {}
                  ...
                  :ops $ let
                      ...
                      trail $ ->
                        ...
                        map $ fn (idx)
                          let
                              t $ * idx $ option:unwrap-or (get state :v) nil
                              dr $ - r1 r2
                              dr2 $ - r2 r3
                              dr3 $ - r3 r4
                              dr4 $ - r4 r5
    ...
    'app.comp.geocentric-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-geocentric-control $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-geocentric-control (state states)
            container
              {} $ :position $ [] 0 -20
              create-list :container ({})
                          ...
                          let
                            ...
                            add-path
                              add-path
                                ...
                                []
                                  * r2 $ phlox.core/ffi-cos $ * t
                                    option:unwrap-or (get state :v2) nil
                                  * r2 $ phlox.core/ffi-sin $ * t
                                    option:unwrap-or (get state :v2) nil
                              []
                                * r3 $ phlox.core/ffi-cos $ * t
                                  option:unwrap-or (get state :v3) nil
                                * r3 $ phlox.core/ffi-sin $ * t
                                  option:unwrap-or (get state :v3) nil
                    concat
                      []
          ...
          :code $ quote $ defcomp comp-grow-demo (touch-key)
            let
              ...
              container ({})
                create-list :container
                  {} $ :position $ [] 700 400
                  -> trails $ map-indexed $ fn (idx trail)
                    [] idx $ graphics $ {}
                      :position $ [] 0 0
        ...
        'generate-trails $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn generate-trails ()
            ...
            let
              ...
              loop
                  idx 50
                  points-with-keys $ map trails $ fn (x)
                    [] (shortid/generate) x
                  acc $ []
                if (= 0 idx)
    ...
    'app.comp.harmono-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-harmono-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-harmono-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
    ...
    'app.comp.oscillo-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-oscillo-control $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-oscillo-control (state states)
            container ({})
              create-list :container ({})
                -> ([] :m :n :step :unit)
                  map-indexed $ fn (idx param)
    ...
    'app.comp.rects-demo $ %{} 'FileEntry
      :defs $ {} $ 'comp-rects-demo
        %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-rects-demo (touch-key)
            container ({})
              create-list :container
                {} $ :position $ [] 40 40
                -> (range 10)
    ...
    'app.comp.reset $ %{} 'FileEntry
      :defs $ {} $ 'comp-reset
        %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-reset (position)
            container
              {} $ :position position
              rect $ {}
                :position $ [] 0 0
    ...
    'app.comp.rotate-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-rotate-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-rotate-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
              ...
              container
                ...
                create-list :container ({})
                  ->
                    ...
                    map $ fn (idx)
                      [] idx $ graphics $ {}
                        :ops $ gen-trail points $ option:unwrap-or (get state :alpha) nil
    ...
    'app.comp.satellite-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-number-controls $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-number-controls (state states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                ...
                create-list :container
                  ...
                  ->
                    ...
                    map-indexed $ fn (idx segment)
                      [] idx $ let
                          r $ + 10 $ * idx
                            option:unwrap-or (get state :unit) nil
                        graphics $ {} $ :ops
                          []
                            g :line-style $ {}
                              :color $ if
    ...
    'app.comp.snowflake-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-snowflake-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-snowflake-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
                ...
                container
                  ...
                  comp-slider (>> states :steps)
                    {}
                      ...
                      :min 0
                      :max $ if
                        >= 3 $ count $ option:unwrap-or (get state :points) nil
                        , 12 6
                      :round? true
                      :on-change $ fn (value d!)
    ...
    'app.comp.sun-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-sun-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-sun-demo (touch-key)
            container
              {} $ :position $ [] 40 200
              comp-reset $ [] -200 -200
              create-list :container
    ...
    'app.comp.tree-demo $ %{} 'FileEntry
      :defs $ {}
        'comp-tree-demo $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-tree-demo (states)
            let
                cursor $ option:unwrap-or (get states :cursor) nil
                state $ or
                  option:unwrap-or (get states :data) nil
